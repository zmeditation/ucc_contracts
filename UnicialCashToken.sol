...
contract UnicialCashToken is ERC20PresetMinterPauser, Ownable, IERC2612 {
    ...
    */
    constructor(uint256 chainId_) ERC20PresetMinterPauser(_tokenName, _tokenSymbol) {
    ...
    function addMinter(address newMinter) onlyOwner external {
        require(owner() != newMinter, "UnicialCashToken: Owner is already has minter role.");
        _setupRole(MINTER_ROLE, newMinter);
    }

    function revokeMinter(address existingMinter) onlyOwner external {
        require(owner() != existingMinter, "UnicialCashToken: Do not revoke owner's minter role.");
